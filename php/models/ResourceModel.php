...
{
    public function getResources($limit = "", $recurso_id = "")
    {
        if(empty( $recurso_id )){
            if(!empty($limit)){
            ...
            if(!empty($limit)){
                return $this->select("SELECT * FROM recursos WHERE id = '" . $recurso_id . "' ORDER BY id DESC LIMIT " . $limit);
            }else{
                return $this->select("SELECT * FROM recursos WHERE id = '" . $recurso_id . "' ORDER BY id");
            }
        }
    ...
    public function setResources($data)
    {
        ...
        if(empty($data["descripcion"]) || empty($data["id_tipo"]) || empty($data["limite"]) || empty($data["precio"])) {
        ...
        return $this->insert("INSERT INTO recursos (descripcion, id_tipo, limite, precio) VALUES ('". $data["descripcion"] . "', '". $data["id_tipo"] . "', '". $data["limite"] . "', '" . $data["precio"] . "')");
    }
