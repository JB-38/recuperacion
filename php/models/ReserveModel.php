...
{
    public function getReserves($limit = "", $reserva_id = "")
    {
        if(empty( $reserva_id )){
            if(!empty($limit)){
            ...
            if(!empty($limit)){
                return $this->select("SELECT * FROM reservas WHERE id = '" . $reserva_id . "' ORDER BY id DESC LIMIT " . $limit);
            }else{
                return $this->select("SELECT * FROM reservas WHERE id = '" . $reserva_id . "' ORDER BY id");
            }
        }  
    }
    ...
    public function setReserves($data)
    {
        ...
        if(empty($data["usuario"]) || empty($data["id_recurso"]) || empty($data["fecha"]) || empty($data["hora"]) || empty($data["cantidad"])) {
            ...
            return ["response" => "Los siguientes campos son requeridos para la creación del elemento: " . $fields, "insert" => FALSE];
        }

        return $this->insert("INSERT INTO reservas (usuario, id_recurso, fecha, hora, cantidad) VALUES ('". $data["usuario"] . "', '". $data["id_recurso"] . "', '". $data["fecha"] . "', '" . $data["hora"] . "', '" . $data["cantidad"] . "')");
    }
